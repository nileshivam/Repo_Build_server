...
namespace MessagePassingComm
{
    ...
    public class Receiver : IMessagePassingComm
    {
        ...
        public void start(string baseAddress, int port)
        {
            ...
            Console.WriteLine("starting Receiver on thread {0}", Thread.CurrentThread.ManagedThreadId);
        ...
        public bool openFileForWrite(string dest, string name)
        {
            try
            {
                string writePath = Path.Combine(dest, name);
    ...
    public class Sender
    {
        ...
        public Sender(string baseAddress, int listenPort)
        {
            ...
            Console.WriteLine("starting Sender on thread {0}", Thread.CurrentThread.ManagedThreadId);
        ...
        public bool postFile(string source, string dest, string fileName)
        {
            ...
            try
            {
                ...
                channel.openFileForWrite(dest, fileName);
                while (true)
                {
            ...
            {
                lastError = ex.Message;
                Console.WriteLine("EXCEPTIOM:::" + ex.Message);
                return false;
            }
            return true;
        }
    ...
    public class TestPCommService
    {
        ...
        {
            Console.WriteLine("testing byte equality for \"{0}\"", filename);
